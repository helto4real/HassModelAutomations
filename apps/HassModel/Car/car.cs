...
[NetDaemonApp]
[Focus]
public class CarHeaterManager : IInitializable
{
    ...
    public CarHeaterManager(IHaContext ha, INetDaemonScheduler scheduler, ILogger<CarHeaterManager> logger, IDataRepository storage)
    {
    ...
    private void HandleCarHeater()
    {
        try
        {
            ...
            // Get relevant states
            var currentOutsideTemp = TempSensor?.AsNumeric().State;
            var configuredDepartureTime = DepartureTimeSensor?.State;
            var scheduleOnWeekend = ScheduleOneWeekendsInputBoolean.IsOn();
            ...
            if (_appJustStarted)
            {
                ...
                _log.LogInformation("The time is {time}, if the time does not match local time, see time zone settings", DateTime.Now);
            ...
            if (HeaterSwitch.IsOn() && !IsManualState)
            {
    ...
    private void TurnOnHeater()
    {
        try
        {
            ...
            if (HeaterSwitch.IsOn())
            {
                ...
                _log.LogInformation("{time} : Turn on heater temp ({temp})", DateTime.Now, currentOutsideTemp.ToString() ?? "unavailable");
                HeaterSwitch?.TurnOn();
            }
        }
        catch (System.Exception e)
    ...
    private void TurnOffHeaterIfOnMoreThanThreeHours()
    {
        try
        {
            if (HeaterSwitch.IsOn())
            {
                if (DateTime.Now.Subtract(HeaterSwitch?.EntityState?.LastChanged ?? DateTime.MinValue) > TimeSpan.FromHours(3))
                {
                    HeaterSwitch?.TurnOff();
                }
            }
